...
public class UserAdministrationController {
    ...
    @GetMapping("")
    public ResponseEntity<?> getUsersPaged(@RequestParam(defaultValue = "20") int size, @RequestParam(defaultValue = "0") int page) throws InterruptedException {
        Thread.sleep(500);
        return ResponseEntity.ok(userService.getAllUsersPaged(PageRequest.of(page, size)).map(adminUserMapper::mapToBasic));
    }
    ...
    @GetMapping("/search")
    public ResponseEntity<Page<AdminBasicUserDataResponse>> adminSearchUsers(@RequestParam(required = false) String query, @RequestParam(defaultValue = "20", required = false) int size, @RequestParam(defaultValue = "0", required = false) int page) {
        return ResponseEntity.ok(userService.findByStringQueryPaged(query, PageRequest.of(page, size)).map(adminUserMapper::mapToBasic));
    }
    ...
    public ResponseEntity<?> getAvailableRoles() {
        return ResponseEntity.ok(new AvailableRolesResponse(Arrays.stream(AppRole.values()).map(AppRole::name).toList()));
    }
