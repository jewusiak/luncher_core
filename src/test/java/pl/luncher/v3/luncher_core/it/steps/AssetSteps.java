...
public class AssetSteps {
  ...
  public void userRequestsCreationOfANewAssetForTheLatestCreatedPlace(List<Map<String, String>> data) {
    ...
    saveHttpResp(getRASpecificationWithAuthAndAcceptHeaders().body(crr).when().post("/asset").thenReturn());
  }
  ...
  public void lastCreatedPlaceHasAsset(int arg0, List<Map<String, String>> assetData) {
    placeRepository.findById(UUID.fromString(getLastCreatedPlaceUuid())).ifPresentOrElse(place -> {
          ...
          var expectedAssets = assetData.stream().map(map -> castMap(map, CommonAssetDb.class)).toList();
  ...
  public void userDeletesLastAssetFromLastCreatedPlace() {
    placeRepository.findById(UUID.fromString(getLastCreatedPlaceUuid())).ifPresentOrElse(place -> {
          ...
          saveHttpResp(
              getRASpecificationWithAuthAndAcceptHeaders().when().delete("/asset/%s".formatted(assetId)).thenReturn());
        },
        () -> {
          throw new EntityNotFoundException();
        });
